...
import { ESCROW_STEPS } from '@scrow/constants';
import { hashCode } from '@scrow/utils';
  ...
  return (
    <Stack spacing="1rem" maxW={maxW} align="stretch">
      <Flex justify="space-between" my={4} align="center">
        ...
        <Heading color="text" size={headingSize}>
          Step {stepNum}: {stepTitle}
        </Heading>

        <Spacer maxW="50px" />
      </Flex>
      <Divider width="100%" background="border" />
      <br />
      {stepDetails.map((detail: string) => (
        <Text color="textMuted" fontSize="sm" key={hashCode(detail)}>
          {detail}
        </Text>
      ))}
    </Stack>
