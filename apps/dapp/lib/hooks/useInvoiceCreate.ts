...
import { FormInvoice, InvoiceMetadata, UseToastReturn } from '@/lib/types';
import {
  errorToastHandler,
...
export const useInvoiceCreate = ({
  ...
  const detailsData = useMemo(() => {
    ...
    return {
      ...
      ...(arbitratorFid && arbitratorUsername
        ? { arbitratorFid, arbitratorUsername }
        : {}),
    } as InvoiceMetadata;
  }, [
  ...
  const {
    ...
    mutation: {
      onSuccess: async hash => {
        ...
        // Notify Farcaster arbitrator if one was selected
        if (arbitratorFid && arbitratorUsername && resolverType === 'custom') {
